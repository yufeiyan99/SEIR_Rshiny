      ...
      sidebarLayout(
        ...
        sidebarPanel(
          ...
          selectInput(inputId = "country",
                      ...
                      choices = as.list(levels(country_list))),
...
server <- function(input, output) {
  ...
  run_model <- reactive({
    ...
    country_confirmed <- country_confirmed[sample(ceiling(length(country_confirmed)*0.1), 1:length(country_confirmed)),]
    country_recover <- data_processing(Recover_Daily) %>% filter(Country.Region==input$country)
    country_recover <- country_recover[sample(ceiling(length(country_recover)*0.1), 1:length(country_recover)),]
